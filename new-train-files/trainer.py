...
def trainer_synapse(args, model, snapshot_path):
    ...
    for epoch_num in range(max_epoch):
        ...
        for i_batch, sampled_batch in tqdm(enumerate(train_loader), total=len(train_loader),
            ...
            loss = 0.4 * loss_ce + 0.6 * loss_dice # Kết hợp cả CrossEntropyLoss và DiceLoss với trọng số 0.4 và 0.6
            optimizer.zero_grad() # Đặt lại gradients trước khi backpropagation
            loss.backward() # Tính toán gradients
            optimizer.step() # Cập nhật trọng số của mô hình
            lr_ = base_lr * (1.0 - iter_num / max_iterations) ** 0.9 # Sử dụng learning rate decay theo công thức: lr = base_lr * (1 - iter_num / max_iterations) ^ 0.9
        ...
        train_loss_total = 0.4*train_ce_loss_epoch+0.6*train_dice_loss_epoch # Tính tổng loss cho epoch hiện tại
